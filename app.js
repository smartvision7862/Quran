...
async function loadQuranReaderVerses() {
  ...
  try {
    ...
    const audioBtnHTML = `<div style="text-align: center; margin-bottom: 20px;"><button class="btn btn-secondary" onclick="playSurahAudio(${readerState.selectedSurah}, '${safeSurahName}')" style="background: rgba(255,215,0,0.1); color: var(--accent-gold); border: 1px solid var(--accent-gold);">▶ Play Surah Audio</button></div>`;
...
async function showTopicVerses(surahId, startAyah, endAyah, topicUr, topicEn) {
  ...
  try {
    const verses = await queryDatabase(
      "quranDb.db",
      "SELECT id, ayat_number, arabic, translation_urdu, translation_english, id - (SELECT COUNT(*) FROM tbl_QuranComplete z WHERE z.ayat_number=0 AND z.id <= t.id) AS global_num FROM tbl_QuranComplete t WHERE surat_id = ? AND ayat_number >= ? AND ayat_number <= ? ORDER BY ayat_number",
    ...
    verses.forEach(v => {
      ...
      item.innerHTML = `
        <div class="wbw-verse-header" style="margin-bottom: 8px;">
          ...
          <button class="wbw-verse-audio-btn" id="topic-audio-btn-${v.global_num}" onclick="toggleVerseAudio(${v.global_num})">▶</button>
        </div>
...
function buildModalBody(lang, type) {
  ...
  const content = {
    ur: {
      ...
      fayde: A('وَمَنْ أَحْسَنُ قَوْلًا مِّمَّن دَعَا إِلَّهِ وَعَمِلَ صَالِحًا وَقَالَ إِنَّنِي مِنَ الْمُسْلِمِينَ', '— سورۃ فصلت: 33') + P('🏆','<strong>ہدایت کا ذریعہ بننا</strong> — ایک شخص کی ہدایت دنیا کی تمام نعمتوں سے بہتر ہے (بخاری)۔') + P('💰','<strong>صدقہ جاریہ</strong> — جب آپ کی دعوت سے کوئی نیک عمل کرے تو قیامت تک ثواب ملتا رہے گا (مسلم)۔') + P('🌟','<strong>انبیاء کا کام سرانجام دینا</strong> — یہ انبیاء علیہم السلام کا عظیم ترین فریضہ ہے۔') + P('❤️','<strong>خود کا ایمان مضبوط ہوتا ہے</strong> — دعوت ایمان کو تازہ رکھتی ہے۔') + P('🤲','<strong>اللہ کی مدد اور برکت</strong> — دعوت دینے والے کے گھر میں برکت آتی ہے۔') + P('🕊️','<strong>سماجی اصلاح</strong> — دعوت سے معاشرہ بہتر ہوتا ہے اور امن آتا ہے۔')
    },
    en: {
...
function attachOrientationListener(qiblaBearing, dialEl, statusEl) {
  ...
  function handleOrientation(event) {
    if (event.type === "deviceorientationabsolute") {
      absoluteFired = true;
    } else if (event.type === "deviceorientation" && absoluteFired) {
      // Ignore relative events if absolute events are firing
      return;
    }
    ...
    if (typeof alpha === "undefined" || alpha === null) {
      ...
      // Convert counter-clockwise alpha to clockwise heading
      alpha = 360 - alpha; 
    }
    
    statusEl.textContent = "Compass Active";
    let targetKaabaDirection = qiblaBearing - alpha;
    
    // Prevent 360 degree spin transition
    let diff = targetKaabaDirection - currentRotation;
...
window.loadMasnoonPdf = function() {
  const container = document.getElementById('pdf-container');
  if (container) {
    container.innerHTML = '<iframe src="masnoon_duain.pdf" width="100%" height="100%" style="border: none;"></iframe>';
  }
};
